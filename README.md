...
## What Lauren can change herself in `/admin.html` — no code, no calling you
...
4. Still in Authentication, go to the **Users** tab → **Add user** → create Lauren's login (her email + a password). This is the only account that can log into `admin.html`.
...
service cloud.firestore {
  match /databases/{database}/documents {
    match /bookings/{bookingId} {
      ...
      // only logged-in admin (Lauren) can read the list or change status
...
This means: the public can create new pending bookings through `book.html`, and can read (but not change) the services menu, gallery, and business settings. Only Lauren (logged in through `admin.html`) can write to any of those, or see/confirm/decline bookings.
...
3. (Optional but recommended) Go to **Domains** → add and verify Lauren's domain, so emails come from `bookings@herdomain.com` instead of Resend's shared test domain. Until that's done, emails can only be sent to Lauren's own verified email — fine for testing, not for real client emails.
...
2. Add environment variables (Project → Settings → Environment Variables, or `vercel env add`):
   ...
   - `LAUREN_EMAIL` — where new-booking notifications should go
