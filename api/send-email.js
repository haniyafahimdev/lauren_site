...
//   vercel env add LAUREN_EMAIL      (where booking notifications go)
...
export default async function handler(req, res) {
  ...
  const LAUREN_EMAIL = process.env.LAUREN_EMAIL || 'haniyafahim.dev@gmail.com' /* TODO: set LAUREN_EMAIL env var on Vercel instead of hardcoding */;
  ...
  if (type === 'new') {
    ...
    if (hasEmail) {
      ...
      html = emailTemplate({
        ...
        ctaUrl: 'https://instagram.com/lauren_does_nails',
        footerNote: 'See you then! Reply to this email or reach out on Instagram @lauren_does_nails if anything changes.'
      });
    } else {
      // No email on file — this person only gave a phone number, so there's
      // nowhere to send *them* a confirmation email. Instead, remind Lauren
...
function emailTemplate({ heading, intro, rows, imagesHtml = '', ctaLabel, ctaUrl, footerNote }) {
  ...
  <div style="background-color:#faf7f0; padding:32px 16px; font-family:Helvetica,Arial,sans-serif;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px; margin:0 auto;">
      <tr>
        <td style="background:linear-gradient(135deg,#1a4d3e,#c9a961); background-color:#1a4d3e; border-radius:20px 20px 0 0; padding:28px 32px; text-align:center;">
          <p style="margin:0; font-family:Georgia,'Times New Roman',serif; font-style:italic; font-size:26px; color:#ffffff; letter-spacing:0.5px;">Lauren</p>
          <p style="margin:4px 0 0; font-family:Helvetica,Arial,sans-serif; font-size:11px; letter-spacing:2px; text-transform:uppercase; color:rgba(255,255,255,0.85);">Nail Tech in Austin, TX</p>
        </td>
      </tr>
      <tr>
        <td style="background-color:#ffffff; padding:32px; border-left:1px solid #e5ddc8; border-right:1px solid #e5ddc8;">
          <h1 style="margin:0 0 10px; font-family:Georgia,'Times New Roman',serif; font-size:22px; color:#1a4d3e; font-weight:normal;">${heading}</h1>
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px 0 6px;">
            <tr>
              <td style="border-radius:999px; background-color:#1a4d3e;">
